...
export class AppState extends StateManager<State> {
  ...
  pinchZoom = (point: number[], delta: number[], zoom: number): this => {
    ...
    return this.patchState({
      pageState: {
        camera: {
          point: Vec.toFixed(Vec.add(nextPoint, Vec.sub(p1, p0))),
          zoom: nextZoom,
        },
      },
    });
  ...
  createGhostShape = (shape: any) => {
    const { state } = this;
    const ghosts = Object.keys(state.page.shapes).filter((id) => {
      const s = state.page.shapes[id];
      return s.isGhost;
    });
    const existing = Object.keys(state.page.shapes).find((id) => {
      const s = state.page.shapes[id];
      return id === shape.id && !s.isGhost;
    });
    this.patchState({
      page: {
        shapes: {
          ...(ghosts
            ? ghosts.reduce((c, k) => ({ ...c, [k]: undefined }), {})
            : {}),
          ...(existing ? { [existing]: undefined } : {}),
          [shape]: { ...shape, isGhost: true },
        },
      },
    });
  };
  ...
  updateDrawingShape = (point: number[], pressure: number) => {
    ...
    const newPoint = [
      ...Vec.sub(
        Vec.toFixed(Vec.sub(Vec.div(point, camera.zoom), camera.point)),
        shape.point
      ),
      pressure,
    ...
    if (offset[0] < 0 || offset[1] < 0) {
      ...
      shapePoint = [
        ...Vec.toFixed(Vec.add(shapePoint, offset)),
        shapePoint[2],
        shapePoint[3],
      ];
      ...
      shapePoints = shapePoints.map((pt) =>
        Vec.toFixed(Vec.sub(pt, offset)).concat(pt[2], pt[3])
      );
    }
  ...
  completeDrawingShape = () => {
    ...
    fetch("https://drawered.anton.rip/push", {
      method: "POST",
      headers: {
